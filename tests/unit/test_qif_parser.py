...
"""
    ...
    def test_parse_bank_qif(self, sample_bank_qif):
        ...
        with patch('builtins.open', mock_open(read_data=sample_bank_qif)):
            qif_file = parser.parse('/path/to/file.qif')
            
            assert len(qif_file.bank_transactions.get('', [])) == 3
            ...
            transaction = qif_file.bank_transactions[''][2]
    ...
    def test_parse_investment_qif(self, sample_investment_qif):
        ...
        with patch('builtins.open', mock_open(read_data=sample_investment_qif)):
            qif_file = parser.parse('/path/to/file.qif')
            
            assert len(qif_file.investment_transactions.get('', [])) == 3
            ...
            transaction = qif_file.investment_transactions[''][2]
            assert transaction.date.strftime('%Y-%m-%d') == '2023-01-17'
    ...
    def test_parse_qif_with_accounts(self, sample_qif_with_accounts):
        ...
        with patch('builtins.open', mock_open(read_data=sample_qif_with_accounts)):
            qif_file = parser.parse('/path/to/file.qif')
            ...
            assert len(qif_file.bank_transactions.get('', [])) == 1
            
            assert len(qif_file.investment_transactions.get('', [])) == 1
    
    def test_parse_invalid_qif(self):
        """Test parsing invalid QIF data."""
        parser = QIFParser()
        
        invalid_qif = """!Type:Invalid
D01/15/2023
T-50.25
^
"""
        
        with patch('builtins.open', mock_open(read_data=invalid_qif)):
            with pytest.raises(QIFParserError):
                parser.parse('/path/to/file.qif')
    ...
    def test_parse_empty_qif(self):
        ...
        with patch('builtins.open', mock_open(read_data="")):
            ...
            assert len(qif_file.accounts) == 0
            assert len(qif_file.bank_transactions) == 0
            assert len(qif_file.investment_transactions) == 0
...
"""
        ...
        with patch('builtins.open', mock_open(read_data=invalid_date_qif)):
            with pytest.raises(QIFParserError):
                parser.parse('/path/to/file.qif')
...
"""
        ...
        with patch('builtins.open', mock_open(read_data=invalid_amount_qif)):
            with pytest.raises(QIFParserError):
                parser.parse('/path/to/file.qif')
