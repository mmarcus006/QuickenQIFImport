...
from quickenqifimport.cli import (
    parse_args, main, get_account_type, load_template_from_arg,
...
"""
    ...
    def test_load_template_from_arg_file(self, sample_template):
        ...
        with patch('os.path.exists', return_value=True), \
             patch('quickenqifimport.cli.load_yaml', return_value=sample_template.dict()), \
    ...
    def test_load_template_from_arg_default(self, sample_template):
        """Test loading default template when name not found."""
        with patch('os.path.exists', return_value=False), \
             patch('quickenqifimport.cli.load_template', side_effect=[ValueError(), sample_template]), \
             patch('quickenqifimport.cli.create_default_templates'):
            
            template = load_template_from_arg('nonexistent', AccountType.BANK)
            assert template == sample_template
        ...
        with patch('quickenqifimport.cli.CSVToQIFService') as mock_service_class:
            ...
            with patch('builtins.open', mock_open(read_data=sample_csv_content)):
                ...
                mock_service.convert_csv_file_to_qif_file.assert_called_once_with(
                    'input.csv', sample_template, 'output.qif', None
                )
